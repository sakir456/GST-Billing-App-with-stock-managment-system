...
const SaleFormItemsTotal = () => {
  const {saleItems, setSaleItems, itemsTotal, setItemsTotal} = useSaleStore()

  useEffect(() => {
    const totalItemQuantity = saleItems.reduce((accumulator, item) => {
      return accumulator + parseFloat(item.qty) || 0;
    }, 0).toFixed(2);
  
    const totalDiscountAmount = saleItems.reduce((accumulator, item) => {
      return accumulator + parseFloat(item.discountAmount) || 0;
    }, 0).toFixed(2);
  
    const totalTaxAmount = saleItems.reduce((accumulator, item) => {
      return accumulator + parseFloat(item.TaxInAmount) || 0;
    }, 0).toFixed(2);
  
    const totalAmount = saleItems.reduce((accumulator, item) => {
